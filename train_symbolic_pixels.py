"""Paper-shaped Craftax Classic pixel training with symbolic DiscoRL policy update."""
...
def softmax(x):
    z = jnp.exp(x - x.max(axis=-1, keepdims=True))
    return z / z.sum(axis=-1, keepdims=True)


def rank01(x):
    r = jnp.argsort(jnp.argsort(-x, axis=-1), axis=-1)
    return r / jnp.maximum(x.shape[-1] - 1, 1)


def symbolic_update_logits(
    current,
    target_out_logits,
    behaviour_logits,
    q,
    ...
    action_dim = current.shape[-1]
    c = center(current)
    pi = softmax(current)
    target_pi = softmax(target_out_logits)
    behaviour_pi = softmax(behaviour_logits)
    rank = rank01(current) - 0.5
    action = jnp.eye(action_dim, dtype=current.dtype)[actions]

    shape_guard = 0.7967 * center(jnp.abs(c)) - 0.0383 * center(c**2) - 0.0032 * center(c**3)
    policy_pull = (
        -1.9514 * center(pi)
        + 1.8500 * center(target_pi)
        + 0.4517 * center(behaviour_pi)
        + 0.1843 * center(target_out_logits - current)
    )
    rank_transport = -0.7203 * center(rank) - 1.9983 * center(rank**2) + 7.8179 * center(rank**3)
    sampled_pg = (
        -0.1919 * center(q)
        - 2.2740 * center(adv[..., None] * action)
        + 0.8456 * center(norm_adv[..., None] * action)
    )
    return center(current) + center(shape_guard + policy_pull + rank_transport + sampled_pg)


def symbolic_meta_out(
    agent: agent_lib.Agent,
    learner_state: agent_lib.LearnerState,
    agent_net_state,
    ...
    agent_out_sg = jax.tree.map(jax.lax.stop_gradient, rollout.agent_out)
    behaviour_out_sg = jax.tree.map(jax.lax.stop_gradient, rollout.behaviour_agent_out)
    target_out_sg = jax.tree.map(jax.lax.stop_gradient, target_out)
    value_outs, adv_ema_state, td_ema_state = value_utils.get_value_outs(
        value_net_out=None,
        target_value_net_out=None,
        q_net_out=agent_out_sg["q"],
        target_q_net_out=target_out_sg["q"],
        rollout=rollout,
        pi_logits=agent_out_sg["logits"],
    ...
    pi_hat = symbolic_update_logits(
        current=agent_out_sg["logits"][:-1],
        target_out_logits=target_out_sg["logits"][:-1],
        behaviour_logits=behaviour_out_sg["logits"][:-1],
        q=jax.lax.stop_gradient(value_outs.target_q_value[:-1]),
        adv=jax.lax.stop_gradient(value_outs.adv),
        norm_adv=jax.lax.stop_gradient(value_outs.normalized_adv),
        actions=actions,
    )
    # The symbolic rule only predicts the policy target. Match y/z to the
    # current network so those auxiliary losses are exactly zero.
    y_hat = agent_out_sg["y"][:-1]
    z_hat = disco_utils.batch_lookup(agent_out_sg["z"][:-1], actions)
    meta_out = dict(
        pi=pi_hat,
        y=y_hat,
        z=z_hat,
        q_target=value_outs.q_target,
        q_value=value_outs.q_value,
        q_td=value_outs.q_td,
        normalized_q_td=value_outs.normalized_q_td,
    )
    meta_out = jax.tree.map(jax.lax.stop_gradient, meta_out)
    coeff = hyper_params["target_params_coeff"]
    new_meta_state = learner_state.meta_state | dict(
        rnn_state=learner_state.meta_state["rnn_state"],
...
def symbolic_learner_step(
    ...
    agent_out, _ = agent.unroll_net(learner_state.params, agent_net_state, rollout)
    ...
    meta_out, new_meta_state = symbolic_meta_out(
        agent, learner_state, agent_net_state, eta_inputs, hyper_params, agent._batch_axis_name
    )
    grads, (_, last_agent_net_state, logging_dict) = jax.grad(agent._loss, has_aux=True)(
        learner_state.params,
    ...
    if agent._batch_axis_name is not None:
        grads = jax.lax.pmean(grads, axis_name=agent._batch_axis_name)
    updates, new_opt_state = agent._optimizer.update(grads, learner_state.opt_state, learner_state.params)
    ...
    return (
        agent_lib.LearnerState(params=new_params, opt_state=new_opt_state, meta_state=new_meta_state),
        last_agent_net_state,
        logging_dict,
    )
...
def save(state: LoopState, cfg: Config, chunk: int, final: bool):
    ...
    payload = dict(
        format="disocraft_symbolic_pixels_v1",
        chunk_idx=chunk,
        final=final,
...
def train(cfg: Config) -> None:
    ...
    run_config |= {
        "env": ENV_NAME,
        "update_rule": "symbolic_policy_from_metaresearch_solution",
        "meta_network": "disabled",
        "y_z_targets": "current_network_self_targets_zero_loss",
    ...
    def train_step(st: LoopState, _):
        ...
        def update(learner):
            ...
            learner, _, m = symbolic_learner_step(agent, step_rng, batch, learner, actor)
