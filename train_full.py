...
PAPER_CRAFTER_ENV_STEPS = 50_000_000
PAPER_CRAFTER_BATCH_SIZE = 24
PAPER_CRAFTER_REPLAY_FRACTION = 0.99
PAPER_CRAFTER_BUFFER_TRANSITIONS = 400_000
PAPER_CRAFTER_ROLLOUT_LEN = 29
NOTEBOOK_DEFAULT_GLOBAL_NUM_ENVS = 4
NOTEBOOK_DEFAULT_GLOBAL_BATCH_SIZE = 256
...
class CraftaxBatchedEnvironment(disco_base_env.Environment):
    ...
    def __init__(self, env_name: str, batch_size: int, auto_reset: bool = True):
        ...
        self._single_action_spec = dm_specs.BoundedArray((), np.int32, 0, action_space.n - 1)
    ...
    def reset(self, rng_key: chex.PRNGKey) -> tuple[CraftaxEnvState, types.EnvironmentTimestep]:
...
def resolve_weights_path(path: str | None) -> Path:
    ...
    pkg = Path(disco_rl.__file__).resolve().parent / "update_rules/weights/disco_103.npz"
    if pkg.exists():
        return pkg

    raise FileNotFoundError("Could not find disco_103.npz. Pass --weights_path or clone disco_rl.")
...
def init_replay_buffer(example_rollout: types.ActorRollout, capacity: int) -> ReplayBufferState:
...
def sample_fifo_replay_batch(
    rng: chex.PRNGKey,
    buffer: ReplayBufferState,
    batch_size: int,
) -> types.ActorRollout:
    """Sample the entire learner batch from FIFO replay, as in eval.ipynb."""
    replay = replay_buffer_sample(buffer, rng, batch_size)
    return swap_time_batch(replay)
...
def sample_mixed_batch(
    ...
    on_indices = jax.random.randint(on_rng, (batch_size,), 0, rollout_bt.rewards.shape[0])
    on_policy = jax.tree.map(lambda x: x[on_indices], rollout_bt)
    replay = replay_buffer_sample(buffer, replay_rng, batch_size)

    use_replay = jax.random.bernoulli(mix_rng, p=replay_fraction, shape=(batch_size,))
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--num_envs", type=int, default=PAPER_CRAFTER_BATCH_SIZE)
    ...
    parser.add_argument("--batch_size", type=int, default=PAPER_CRAFTER_BATCH_SIZE)
    parser.add_argument("--updates_per_iter", type=int, default=1)
    parser.add_argument("--replay_mode", choices=("mixed", "fifo"), default="mixed")
...
def resolve_num_iterations(
    ...
    if target_env_steps is None or target_env_steps <= 0:
        raise ValueError("target_env_steps must be positive when num_iterations is omitted.")
    if steps_per_iter <= 0:
        raise ValueError("steps_per_iter must be positive.")

    raw = target_env_steps / steps_per_iter
    if rounding == "floor":
        resolved = math.floor(raw)
    elif rounding == "ceil":
        resolved = math.ceil(raw)
    elif rounding == "nearest":
        resolved = round(raw)
    else:
        raise ValueError(f"Unsupported iteration_rounding={rounding!r}.")
...
def train(args: argparse.Namespace) -> None:
    ...
    actual_target_steps = num_iterations * steps_per_iter

    if args.buffer_capacity is None:
        requested_capacity = int(math.ceil(args.buffer_capacity_transitions / args.rollout_len))
    else:
        requested_capacity = int(args.buffer_capacity)
    if requested_capacity <= 0:
        raise ValueError("buffer_capacity must be positive.")

    max_chunks_needed = args.num_envs * num_iterations
    ...
    replay_ratio = (args.batch_size * args.updates_per_iter) / args.num_envs
    fifo_replay_fraction = 1.0 - (1.0 / replay_ratio)
    print(
        "Paper/eval schedule: "
        ...
        f"actual_env_steps={actual_target_steps}"
    )
    print(
        "Replay mix: "
        ...
        f"fifo_replay_ratio={replay_ratio:.2f} "
    ...
    def train_loop(state):
        return jax.lax.scan(train_step, state, xs=None, length=num_iterations)
