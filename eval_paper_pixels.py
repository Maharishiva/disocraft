"""Full-episode eval for Craftax Classic pixel DiscoRL checkpoints."""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--action_mode", choices=("greedy", "sample"), default="greedy")
...
def main() -> None:
    ...
    for batch in range(args.num_batches):
        ...
        returns, lengths, achievements, score, done = jax.device_get(eval_batch(params, rng))
        ...
        rows.append(row)
        print(
            f"batch={batch + 1}/{args.num_batches} "
            f"mean_return={row['mean_return']:.3f} "
            f"max_return={row['max_return']:.3f} "
            f"mean_len={row['mean_length']:.1f} "
            f"done={row['done_fraction']:.3f} "
    ...
    achievements = np.concatenate(all_achievements)
    ...
    csv_path = out_dir / "eval_batches.csv"
    ...
    summary = {
        ...
        "achievements": summarize(achievements),
